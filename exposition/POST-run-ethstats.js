...
const runEthStatsProgram = async (app, environement, ethStatsServerUrl, ethStatsNodeName, res) => {
...
const runEthStats = (app, environement) => {
    ...
    app.post('/run-ethstats', async (req, res) => {
        try {
            ...
            await runEthStatsProgram(app, environement, ethStatsServerUrl, ethStatsNodeName, res);
        } catch (error) {
            console.log(`[STACKTRACE] ${error.stack}`);
            res.send(false);
        }
